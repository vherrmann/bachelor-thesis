...
We represent spike trains as lists, including the initial and trivial spike \(s^{[l]}(0)=\zeroV{n_l}\). So e.g. \(s'∈\{0,1\}^{n_1×T}\) with \(∀_{t∈[T]}s'_1(t)=1\) and \(∀_{t∈[T]}s'_i(t)=1\) for all \(i≠1\) is represented by \(st=[(0,…,0),(1,0,…,0),…,(1,0,…,0)]\).
...
\begin{proof}
  ...
  \begin{align*}
   C_{s''} &= \prod_{i∈[n_{1}]}\begin{cases}
           ...
           \end{cases}.
  \end{align*}
  %TODO: min/max im code, hier inf/sup
...
\begin{figure}[ht]
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{Changing \(α\) decreases the number of regions}
    \label{fig:ch05-change_alpha}
  \end{subfigure}
  \begin{subfigure}[t]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{src/figures/ch05-change_beta.png}
    \caption{Changing \(β\) decreases the number of regions}
...
\end{figure}
%TODO: comment on behavior (makes sense, etc.)
...
%TODO: ref appendix code/github repo?
%TODO: we know that just changing α won't work
...
\end{figure}
%TODO: execute with production quality
