...
%TODO: remark about comparative strength of dt.lifsnn and rsnn, compare images

% TODO: what kind of shapes a d.t. …
% TODO: what is meant with graph
% TODO: etwas text zwischen aussagen
In the following we will analyze what shape the graph of a \rdtlifsnn has. Of particular interest to us is the question how many different values a \rdtlifsnn can produce.
...
We will further generally assume \(W^{[1]}=\idM{n_1}\). Let \(Φ\) be a \rdtlifsnn with \(W^{[1]}\) arbitrary and \(Φ'\) the same one, but with \(W^{[1]}=\idM{n_1}\). We then have \(R(Φ)(x)=R(Φ')(W^{[1]}x)\) since we are using direct encoding. So \(W^{[1]}\) just corresponds to a pre-transformation on the input vector and can therefore (1) only decrease the number of different output values and further the key to understanding the output landscape of a general \rdtlifsnn is to understand the landscape of one with \(W^{[1]}=\idM{n_1}\).

We can further simplify the notation in this section by writing \(W,b,V,α,β,ϑ,i,u,s,n\) for \(W^{[1]},b^{[1]},V^{[1]},α^{[1]},β^{[1]},ϑ^{[1]},i^{[1]},u^{[1]},s^{[1]},n_1\) respectively, since we will only be working on the first layer anyways. Further write \(x≔s^{[0]}\). Since we are using direct encoding, we may assume \(∀_{t∈[T]}s^{[0]}(t)=x\). We now get the following simplified defining equations:
\begin{align}
...
By using the simplified notation, we obtain the following definitions from~\cref{def:non-recursive-defs} for the input vector \(x∈ℝ^{n_0}\) and the first hidden layer spike-train \(σ\):
\begin{align}
    ...
    u(t;x;σ) &= β^tu(0)+\sum_{k=1}^tβ^{t-k}\left(i(k;x;σ)+b-ϑσ(k)\right), \label{eq:2}
\end{align}
...
While quite technical, \(g\) is the key to proper rigorous proofs about the landscape of \rdtlifsnns.

\begin{lemma}\label{lem:invert-p}
  There is a function \(g:\dot{\bigcup}_{t∈[T]}\{σ\mid σ∈\{0,1\}^{n_1×(t-1)}\}→ℝ\) such that \(∀_{i∈[n_1]}s_i(t;x;σ)=1⇔⟨w_i,x⟩≥g_i(t;σ)\) defined by
  ...
  Where \(w_i\) denotes the \(i\)-th row vector of \(W\). %MAYBE: remove?
\end{lemma}
...
\begin{remark}\label{rem:g-prefix}
  Like with \(i(t;x;σ)\), \(p(t;x;σ)\), etc. we will allow supplying \(g\) with a extension of the spiketrain that is required. In that case the value of \(g\) should be understood as the value at the prefix of that spiketrain.
\end{remark}
...
\begin{proposition}\label{prop:const-regions-cuboids}
  ...
  using the conventions \(\inf ∅=∞\) and \(\sup ∅=-∞\) here. %MAYBE: put to beginning
\end{proposition}

\begin{remark}\label{rem:const-regions-cuboids}
  Note that by \cref{prop:const-regions-cuboids}, we have \(x^{s'}_i=-∞\) exactly if \(∀_{t∈[T]}s'_i(t)=0\); and \(y^{s'}_i=∞\) exactly if \(∀_{t∈[T]}s'_i(t)=1\). So in particular we cannot have \(s'\) with an component \(i∈[n_1]\), such that both \(x^{s'}_i=-∞\) and \(y^{s'}_i=∞\).
\end{remark}
...
\begin{proof}[\proofofref{lem:const-regions-cuts}]
  ...
  \end{align*}
  by~\cref{lem:non-recursive-defs} for the second equality and~\cref{lem:invert-p} for the last one.
\end{proof}
...
\begin{lemma}\label{lem:intersection-cuboid}
  ...
  We use \(\inf/\sup\) instead of \(\min/\max\) to highlight that the components of the vertices might be \(±∞\).
\end{lemma}
...
\begin{definition}
  Let \(s',s''∈\{0,1\}^{n_1×T}\), we then have \(s'≤_ls''\) exactly if either \(s'=s''\) or there exists a \(t∈[T]\) such that \(s'(t)<s''(t)\) and \(∀_{t'<t}s'(t')=s''(t')\).
  We further define \(s'<_ls''\) as \(s'≤_ls''\) and \(s'≠s''\).
\end{definition}

\begin{lemma}\label{lem:spike-trains-ineq}%TODO: introduce s(·;y)
  Let \(x,y∈ℝ^{n_0}\). We then have \(x≤y⇒s(·;x)≤_ls(·;y)\).

  Is on the other hand \(s(·;x)<_ls(·;y)\) and \(i∈[n_1]\) such that \(s_i(t;x)≠s_i(t;y)\) holds at the minimal time \(t\) at which \(s(·;x)\) and \(s(·;y)\) are different, then \(x_i<y_i\) holds.
\end{lemma}
...
\begin{proof}
  First notice that for a fixed \(σ\) the function \(i_i(t;x;σ)\) is growing montonically in \(x_i\) for all \(i∈[n]\), since \(α≥0\). We similarly get that \(p_i(t;x;σ)\) and \(s_i(t;x;σ)\) are growing monotonically in \(x_i\), since \(H=χ_{[0,∞)}\) is monotonically growing.

  Let now \(x≤y\) and suppose \(s(·;x)≠s(·;y)\). We then have a minimal \(t∈[T]\) such \(s(t;x)≠s(t;y)\). Now due to \(∀_{t'<t}s(t';x)=s(t';y)\) and \(x≤y\) we have \(∀_{i∈[n_1]}s_i(t;x)≤s_i(t;y)\) using the previous remark with \(σ(t)≔s(t;x)\). We therefore get \(s(t;x)<_ls(t;y)\).
...
\begin{definition}
  ...
  A spike train \(s'\) is therefore constant in every component, if \(∀_{i∈[n_1]}∀_{t,t'∈[T]}s'_i(t)=s'_i(t')\).
  We further call \(s'\) non-constant in every component, if \(∀_{i∈[n_1]}∃_{t,t'∈[T]}s'_i(t)≠s'_i(t')\).
\end{definition}

We also need some geometric definitions.

\begin{definition}
  We call a point \(p∈(ℝ∪\{±∞\})^n\) vertex of a non-empty region \(⟦x,y⦆\), \(⟦x,y⟧\) or \(⦅x,y⦆\), if \(∀_{i∈[n]}p_i∈\{x_i,y_i\}\). We call a vertex \(p\) finite, if \(p∈ℝ^{n}\).
\end{definition}
...
\begin{remark}
  So in particular, every region \(C_{s'}∈C_{Φ,1}\) has at least one finite vertex.
\end{remark}
...
\begin{proof}
  ...
  for the set of finite vertices, \(ℝ^{n_1}∩\prod_{j∈[n_1]}\{x^{s'}_j,y^{s'}_j\).
\end{proof}

\begin{lemma}\label{lem:non-finite-regions}
  Let \(s'∈\{0,1\}^{n_1×T}\) be a spike train such that \(C_{s'}≠∅\). Then the following conditions,
  ...
  \item \(C_{s'}\) has a non-finite vertix,
...
\begin{proof}
  ...
  vertices, so assuming \(C_{s'}\) has \(≤2^{n_1-1}\) finite vertices, it must have a non-finite one.
  \item \((c)⇒(b)\): If \(C_{s'}\) has a non-finite vertice, then the number of finite vertices is smaller than \(2^{n_1}\). On the other hand the number of finite vertices must be a power of \(2\) by~\cref{lem:constant-components-finite-vertices}, so the number must be smaller or equal \(2^{n_1-1}\).
  \item \((c)⇔(d)\): If all vertices are finite, then in particular \(x^{s'},y^{s'}∈ℝ^{n_1}\) and \(⟦x^{s'},y^{s'}⦆\) is clearly bounded. 
  Is one vertice non-finite, then either \(x^{s'}\) or \(y^{s'}\) has a component that is \(±∞\). Suppose we have \(x^{s'}_i=-∞\). Since \(C_{s'}≠∅\) we further have \(z∈C_{s'}\). By definition of \(⟦x^{s'},y^{s'}⦆\), we have now \(z'∈⟦x^{s'},y^{s'}⦆\) with \(∀_{i≠j}z'_j=z_j\) and \(z'_i∈[-∞,z_i]\) arbitrary. So \(C_{s'}\) is not bounded. The proof for \(y^{s'}_i=∞\) is similar.
  \end{itemize}
\end{proof}
...
\begin{proposition}\label{prop:finite-vertices-bounds}
  ...
  %TODO: idea for x^C_i: leftmost border of the left-mostest regions
  ...
  We further have in particular \(x^C_i = \min_{t∈[T]}g_i(t;s^{x^C_i})\) and \(y^C_i = \max_{t∈[T]}g_i(t;s^{y^C_i})\), where
  \begin{align*}
...
\begin{remark}
  From~\cref{prop:finite-vertices-bounds} we obtain in particular, that we \(C_{s'}⊂⟦x^C,y^C⦆\) for all regions \(C_{s'}∈C_{Φ,1}\) with non-constant spiketrain \(s'\):
...
\begin{remark}
  \cref{prop:finite-vertices-bounds} is in particular useful, when computing \(\abs{C_{Φ,1}}\): We know that every region \(C_{s'}∈C_{Φ,1}\) has at least one finite vertex by~\cref{lem:constant-components-finite-vertices} and by~\cref{prop:finite-vertices-bounds} we know where to search for those vertices.
\end{remark}

\begin{proof}
  We will first proof \(P⊂⟦x^C,y^C⦆\): Let \(v∈P\) be a finite vertix of region \(C_{s'}=⟦x^{s'},y^{s'}⦆\). Now \(∀_{i∈[n_1]}v_i∈\{x^{s'}_i,y^{s'}_i\}∩ℝ\), so in particular in the case of \(v_i=x^{s'}_i\) for a \(i∈[n_1]\) there is time-step \(t_1∈[T]\) such that \(s'(t_1)=1\); is instead \(v_i=y^{s'}_i\) then there is time-step \(t_0∈[T]\) such that \(s'(t_0)=0\). Let now \(i∈[n_1]\) and \(v_i=x^{s'}_i\) be given. Let further \(t_1\) be minimal with \(s'(t_1)=1\).
  Let us now define \(s^1∈\{0,1\}^{n_1×t_1}\) by \( s^0(t)≔\zeroV{n_1} \). Now by choice of \(t_1\), we have  \(∀_{t<t_1}s^1_i(t)=0\). Further, since \(g(t;σ)\) only consumes the first \(t-1\) time-steps of the given spike train \(σ\) (see~\cref{rem:g-prefix}), we have \(g_i(t_1;s^0)=g_i(t_1;s')\). We therefore get
  ...
  Let us now show that it suffices to compute \(g\) on \(s_j^{x^C_i}\) and \(s_j^{y^C_i}\) to obtain the boundaries. For that purpose we have to take another look at the definition of \(g\). Since the definition is quite clunky we have split it up into the following functions
  \begin{align*}
    g^r_i(t;σ)&≔-\frac{\sum_{k=1}^tβ^{t-k}\left(α^ki_i(0)+b+\sum_{l=1}^kα^{k-l}⟨v_i,σ(l-1)⟩\right) +β^tu_i(0)}{\sum_{k=1}^tβ^{t-k}\sum_{l=1}^kα^{k-l}}, \\
    g^{ϑ}_i(t;σ)&≔\frac{ϑ\left(1+\sum_{k=1}^{t-1}β^{t-k}σ_i(k)\right)}{\sum_{k=1}^tβ^{t-k}\sum_{l=1}^kα^{k-l}}
  \end{align*}
  such that \(g(t;σ)=g^{r}(t;σ)+g^ϑ(t;σ)\). Now fixing \(t\) and requiring \(∀_{t∈[T]}σ_i(t)=0\), the function \(g^ϑ_i(t;σ)\) constant and \(g^r_i(t;σ)\) is minimal for \(σ_j(t)=1_{((v_i)_j>0)}\) for \(i≠j\), so in particular for \(σ=s^{x_i^C}\). Hence we indeed have \(x^C_i = \min_{t∈[T]}g_i(t;s^{x^C_i})\). We similarly get \(x^C_i = \min_{t∈[T]}g_i(t;s^{y^C_i})\).
  %TODO: check if g_i or g
\end{proof}
...
\begin{theorem}\label{thm:regions-upper-bound}
  Consider a discrete-time LIF-SNN \(Φ\) with \(T\) time steps, input dimension \(n_0\) and \(n_1\) neurons in the first hidden layer. Then the number of constant regions is bounded by
  \begin{equation}
...
\begin{proof}
  ...
  Let us further proof \(\abs{C_{Φ,1}}≤ \left(\frac{T^2+T+2}{2}\right)^{n_1}\). In the following we will use \(Φ_τ\) to mean the \rdtlifsnn that only differ from \(Φ\) by \(T_{Φ_τ}=τ\). It suffices now to show \(\abs{C_{Φ_{τ},1}}≤ \left(\frac{τ^2+τ+2}{2}\right)^{n_1}\) inductively.
  Let first \(\)
\end{proof}

% TODO: α does not increase

We will now try to give a high-level overview on why this is the case. We will assume \(W=I_{n_1}\) again.
 The proof in~\cite{nguyen2025timespikeunderstandingrepresentational} makes use of two major facts:
% \begin{itemize}
%   \item For \(W=\idM{n_1}\), \(V=\zeroV{n_1×n_1}\) and \(α=0\) we get equations
%     \begin{align}
%     i(t) & = x \\
%     p(t) & = βu(t-1)+i(t)+b \\
%     s(t) & = H(p(t)-ϑ\oneV{n}) \\
...
%     and in particular
%     \begin{align}
%         p(t;x) &= β^tu(0)+(x+b)\sum_{k=1}^tβ^{t-k}-ϑ\sum_{k=1}^{t-1}β^{t-k}s(k)
%     \end{align}
...
\begin{figure}[!htbp]
  ...
  \end{subfigure}
  \caption{Comparison of \dtlifsnn vs. \rdtlifsnn; White lines separate regions with a different spike count; Pink lines separate regions with a different spike count disregarding the latest spikes from the last times-step}
\end{figure}
...
While we were able to get promising empirical results in~\cref{ch:experiments} that seem to confirm that\cref{thm:regions-upper-bound} also holds for \rdtlifsnn, we were not able to proof them.
...
\begin{itemize}
  \item For \(W=\idM{n_1}\), \(V=\zeroV{n_1×n_1}\) and \(α=0\) we get equations
    ...
    Notice that the components \(i_i,p_i,s_i,u_i\) are computed in parallel, without affecting each other, in particular due to \(V=0\).
        ...
        , the neurons of the first layer don't influence each other in \dtlifsnn and the proof can therefore just focus on how many different spikestrains a single neuron can possibly emit. Due to, each neurons input has its own independent input component. So the number of different values the whole first hidden layer can emit is just the product of the maximal number of spikes each neuron can emit. Since the parameters can be arbitrary, that maximal number is the same for each neuron. This is why the upper bound in~\cref{thm:regions-upper-bound} has an exponent \(n_1\).
\item Let us regard the definition of \(g\) for \(α=0\) and \(V=0\):
  \[ g(t;σ)=-b-\frac{β^tu(0)-ϑ\left(1+\sum_{k=1}^{t-1}β^{t-k}σ(k)\right)}{\sum_{k=1}^tβ^{t-k}} \]
  Let us further regard a time-step \(t∈[T]\). If we regard the spiketrains \((s_i(t';x))_{t'∈[t]}\) we get for \(x∈ℝ^{n_1}\), we see that \(s_i(·;x)\) only changes depending on \(x_i\), since \(V=0\). Further, if we have \(x,y∈ℝ^{n_1}\) such that \(∀_{i≠j}x_j=y_j\) with \(x_i<y_i\) and \(s_i(·;x)≠s_i(·;y)\), then  \(s_i(·;x)<s_i(·;y)\) lexiographically by~\cref{lem:spike-trains-ineq}. In fact, not only lexiographically, but even stronger, but every spike appears earlier in \(s_i(·;y)\) than in \(s_i(·;x)\).
        %TODO: clear up notation s_i(·;…) vs. ()_t∈[T]

  If we now regard \(g\), we therefore see that \(g_i(t;s_i(·;x))≥g_i(t;s_i(·;y))\). So the ordering has been reversed. But since the points returned by \(g\) define the points where the regions are split (compare~\cref{prop:const-regions-cuboids}), this means that only one region among all of those that share the same number of spikes will be split in time-step \(t\). Since there are only \(t+1\) possible counts of spikes at \(t\), we therefore only get \(t+1\) possible splits when going from \(t\) to \(t+1\), wherefore we get \(≈\sum_{t=1}^Tt=\frac{T^2+T}{2}\) regions in this neuron.
\end{itemize}

%TODO: images (β≠1+V≠0, β=1+V≠0)

Now since the neurons of the first layer of a \rdtlifsnn can influence each other, it does not suffice to look at the behavior of a single neuron. We instead have to look at all the neurons together. One approach that seemed quite promising was to focus on partitioning the input space into regions with the same number of spikes in every component. This gives some control onto the next regions, but still not enough to have the same reversed orderings of regions in \(C_{Φ,1}\) and points induced by \(g\): Consider …
Further, even for \(β=1\), where we clearly get that \(g\) is constant on the whole region with constant number of spike trains, we don't quite have enough enough control on the behavior of the new regions.
% TODO: refer to images
% crosses don't necessarily cross only so many

% CANCELLED: remove lem 4.7?
% TODO: more time-steps don't really increase the region that is getting fitted upon
