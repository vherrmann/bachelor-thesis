\section{Definition of \rdtlifsnn}
\label{ch:defs}

\subsection{Notations and Conventions}
First a few words about the notation we will use in the paper:

\newcommand{\rangeI}[2]{\{#1,…,#2\}}
We write \(\rangeI{a}{b}≔\{a,a+1,…,b\}\) for the range of integers from \(a\) to \(b\) and in particular \([n]≔\rangeI{1}{n}\) for the integers from \(1\) to \(n\) and \([n]_0≔\rangeI{0}{n}\) for the integers from \(0\) to \(n\). Moreover \(∨\) is used to mean the logical “or”, \(∧\) to mean the logical “and”.

We write \([x,y)≔\{z∈ℝ\mid x≤z<y\}\) for the half-open interval between \(x∈ℝ\) and \(y∈ℝ\) and further use \(⟦x,y⦆\) with \(x,y∈ℝ^n\) to write half-open cuboids \(\prod_{i=1}^n[x_i,y_i)\). Similarly \(⟦x,y⟧\) is the closed cuboid \(\prod_{i=1}^n[x_i,y_i]\). A cube is a cuboid such that all sides have the same length. We further take \([x,y)\) to be empty for \(x,y∈ℝ\) with \(x≥y\), and \([-∞,x)]\) to mean \((-∞,x)\). This means in particular, that \(⟦x,y⦆=∅\) holds exactly when \(y_i≤x_i\) for a \(i∈[n]\). 

We further use \(\operatorname{diam}_p(U)≔\sup_{x,y∈U}\norm{x-y}_p\) to mean the diameter of \(U⊂ℝ^m\) regarding the \(p\)-norm \(\norm{·}_p\).

\newcommand{\idM}[1]{I_{#1}}
\newcommand{\zeroV}[1]{\mathbf{0}_{#1}}
\newcommand{\oneV}[1]{\mathbf{1}_{#1}}
Continuing, \(e_i≔(0,…,1,…,0)∈ℝ^n\) is the \(i\)-th standard basis vector, \(\zeroV{n}≔(0,…,0)∈ℝ^n\), \(\oneV{n}≔(1,…,1)∈ℝ^n\) the vectors containing \(0\) and \(1\)s in every component, respectively and \(\idM{n}∈ℝ^{n×n}\) the identity matrix of size \(n×n\). Is moreover \(W∈ℝ^{n×m}\) a matrix, then we will write \(w_i∈ℝ^{1×n}\) for the \(i\)-th row vector and \(W_{i,j}∈ℝ\) for the value of the cell \((i,j)\). We add an ordering \(≤\) to vectors: \(x≤y\) holds exactly for \(x,y∈ℝ^n\) if \(∀_{i∈[n]}x_i≤y_i\). Further \(x<y\) is canonically defined, so \(x<y\) holds exactly if \(x≤y\) but not \(x=y\).

Is further \(f:U→ℝ^n\) a function with arbitrary domain \(U\), then \(f_i≔π_i∘f\) is the \(i\)-th component function. \(π_i:ℝ^n→ℝ\) is the projection to the \(i\)-th component. We also write \(f^{-1}\) for the inverse of \(f\) and \(f^{-1}(W)≔\{x∈U \mid f(x)∈W\}\) for the preimage of \(W\) under \(f\). The set of continuous functions of type \(U→W\) will be written by \(𝒞^0(U,W)\). Correspondingly, the set of \(k\)-times continuously differentiable functions will be written \(𝒞^k(U,W)\). The total derivative of a differentiable function \(f:U→W\) is written as \(df:U→\operatorname{Hom}_{ℝ}(W,W)\).

The norms \(\norm{·}_p\) either are the \(p\)-norm on \(ℝ^n\) or the operator norm with regard to the \(p\)-norm \(\norm{·}_p\) in input and output space. Further, \(\norm{f}_{∞,p}≔\sup_{x∈U}\norm{f(x)}_p\) for a function \(f:U→ℝ^n\).

Some further notation regarding geometry: \(\overline{A}\) is the closure of \(A⊂ℝ^n\), \(A°\) the interior; we further write \(B_{ε,p}(x)≔\{y\mid \norm{y-x}_p<ε\}\) for \(ε>0\), \(p≥1\) and \(x∈ℝ^n\).

We moreover write \(\max{U}≔(\max_{u∈U}u_i)_{i∈[n]}\) with \(U⊂(ℝ∪\{±∞\})^n\) for the maximum by component of \(U\), we similarly define \(\min{U}\), \(\inf{U}\) and \(\sup{U}\). In addition we use the conventions \(\inf(∅)=∞\) and \(\sup(∅)=-∞\). We will also use \(∧\) and \(∨\) to write the supremum and infimum respectively when convenient.

%DONE: f^-1 for the inverted function, f^-1(U) for preimage
%DONE: define < for vectors (convention)

%DONE: notation for total der.

%MAYBE: introduce notation []_…^… as product
%DONE: notation \(\norm{f}_{∞,2}\), \(\norm{g}_2\) für \(g\) linear

%DONE: notation \(\overline{}\), °, closure, interior
%CANCELLED: introduce notation N_ρ(Ω)
%DONE: introduce ∨,∧ notation

\newcommand{\charac}[1]{χ_{#1}}
\newcommand{\oneP}[1]{1_{#1}}
We finally write
\[\charac{M}(x)≔\begin{cases} 1 & x∈M \\ 0 & x∉M \\ \end{cases}\] %CANCELLED: change notation? e.g. 1_M instead?
for the characteristic function of a set \(M\) and
\[\oneP{A}≔\begin{cases} 1 & A \\ 0 & ¬A \\ \end{cases}\] %CANCELLED: change notation? e.g. ⟦A⟧ instead
for a formula \(A\), such that in particular \(\charac{M}(x)=\oneP{(x∈M)}\).

\subsection{Motivation}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.7\linewidth]{src/figures/ch02-structure-of-neuron.png}
  \caption{Structure of a neuron.\protect\footnotemark}
  \label{fig:ch02-structure-of-neuron}
\end{figure}
\footnotetext{Source: By BruceBlaus - Own work, CC BY 3.0, \url{https://commons.wikimedia.org/w/index.php?curid=28761830 }}

%DONE: hier überarbeiten

A neuron as in~\cref{fig:ch02-structure-of-neuron} has been described in~\cite{neuronaldynamics2014} by the following:

\begin{displayquote}
  A typical neuron can be divided into three functionally distinct parts, called dendrites, soma [also called cell body], and axon; […]. Roughly speaking, the dendrites play the role of the ‘input device’ that collects signals from other neurons and transmits them to the soma. The soma is the ‘central processing unit’ that performs an important non-linear processing step: If the total input arriving at the soma exceeds a certain threshold, then an output signal is generated. The output signal is taken over by the ‘output device’, the axon, which delivers the signal to other neurons.
\end{displayquote}

The nature of the signals traveling between neurons is described in a following paragraph.

\begin{displayquote}
  The neuronal signals consist of short electrical pulses […]. The form of the pulse does not change as the action potential propagates along the axon. A chain of action potentials emitted by a single neuron is called a spike train – a sequence of stereotyped events which occur at regular or irregular intervals; […]. Since isolated spikes of a given neuron look alike, the form of the action potential does not carry any information. Rather, it is the number and the timing of spikes which matter. 
\end{displayquote}

Finally, the effect of signals on the state of the cell body is outlined in the last quote.

\begin{displayquote}
  The effect of a spike on the postsynaptic neuron can be recorded with an intracellular electrode which measures the potential difference u(t) between the interior of the cell and its surroundings. This potential difference is called the membrane potential. Without any input, the neuron is at rest corresponding to a constant membrane potential urest. After the arrival of a spike, the potential changes and finally decays back to the resting potential, […].
\end{displayquote}

\begin{comment}
Finally the connection between two neurons has a small gap, called synapse:

\begin{displayquote}
  The site where the axon of a presynaptic neuron makes contact with the dendrite (or soma) of a postsynaptic cell is the synapse. […] At a chemical synapse, the axon terminal comes very close to the postsynaptic neuron, leaving only a tiny gap between pre- and postsynaptic cell membrane. This is called the synaptic cleft. When an action potential arrives at a synapse, it triggers a complex chain of bio-chemical processing steps that lead to a release of neurotransmitter from the presynaptic terminal into the synaptic cleft. As soon as transmitter molecules have reached the postsynaptic side, they will be detected by specialized receptors in the postsynaptic cell membrane […].
\end{displayquote}
\end{comment}

To summarize: A neuron in the human brain has connections to other neurons, which may be weaker or stronger, as well as an internal membrane potential that is increased or decreased by the input from other neurons. If that membrane potential reaches a threshold, the neuron will spike and the subscribed neurons will receive an update. If it does not reach the threshold, the potential just decays to the rest potential. Updates from other neurons are delivered by chemical transmitters or electric signals. We will not model these signals as a single binary spike, such that in particular the input immediately drops down to the rest state, but will instead let it decay over time after the spike, e.g.\ because the chemical transmitters have some chance of taking a longer path to the next neuron.

From these biologic observations we can derive the following differential equations for a single neuron input \(I:ℝ→ℝ\) and membrane potential \(U:ℝ→ℝ\):
%DONE: work in {neuronaldynamics2014} and figure

% DONE: graphic, https://en.wikipedia.org/wiki/Neuron#/media/File:Blausen_0657_MultipolarNeuron.png ?
% DONE: link paper for equations
\begin{align*}
 I'(t) &≔-τ_αI(t)+\sum_{j=1}^nw_is_i(t-Δt) \\
 U'(t) &≔-τ_βU(t)+I(t)+b-ϑs(t)
\end{align*}
Here \(s_i:ℝ→\{0,1\}\) represents the \(j\)-th connection of the given neuron spiking at time \(t\); the variables \(w_i∈ℝ\) represent the weights of the connections. The \(ϑs(t)\) resets the potential \(U(t)\) after a spike. Since we might get unresolvable dependencies between neurons otherwise, the connections need to have some latency \(Δt∈ℝ\). The variables \(τ_{α},τ_{β}>0\) specify the decay rate with which \(I\) and \(U\) respectively decay.
We also include the bias \(b∈ℝ\) in the differential equation of \(U\) to simplify constructions of \rdtlifsnn later. %While not so much neurologically motivated, the bias \(b\) can be replaced by a constantly spiking neuron adding that bias through the input \(I\).

%DONE: motivate bias

Since we are in an analog scenario, it is reasonable to assume that \(I\) and \(U\) describe smoothly differentiable functions. We can therefore use the first-order exponential integrator method to obtain a discretization of \(I\) and \(U\) from the differential equations. Let \(t_0,h∈ℝ\) be arbitrary and \(t_{n+1}≔t_n+h\). By using the fundamental theorem of calculus we get
\begin{align*}
  &e^{τ_{α}t_{n+1}}I(t_{n+1})-e^{τ_{α}t_n}I(t_n) \\
  &=\int_{t_n}^{t_{n+1}}\frac{d}{dt}\left(e^{τ_{α}t}I(t)\right)dt \\
  &=\int_{t_n}^{t_{n+1}}e^{τ_{α}t}(I'(t)+τ_αI(t))dt \\
  &=\int_{t_n}^{t_{n+1}}e^{τ_{α}t}\left(\sum_{j=1}^nw_is_i(t-Δt)\right)dt
\end{align*}
If we assume the input from the spikes of other neurons to be constant during \([t_n,t_{n+1}]\), we get
\begin{align*}
 &=\frac{1}{τ_{α}}(e^{τ_{α}t_{n+1}}-e^{τ_{α}t_n})\sum_{j=1}^nw_is_i(t-Δt) \\
 &=\frac{1}{τ_{α}}e^{τ_{α}t_{n+1}}(1-e^{τ_{α}h})\sum_{j=1}^nw_is_i(t-Δt)
\end{align*}
So we get
\[ I(t_{n+1})=e^{-τ_{α}h}I(t_n)+\frac{1}{τ_{α}}(1-e^{τ_{α}h})\sum_{j=1}^nw_is_i(t-Δt) \]
Let us now use \(h=1\) and absorb \(\frac{1-e^{τ_{α}}}{τ_{α}}\) into the weights \((w_i)_{i∈[n]}\), such that we have
\begin{equation}\label{eq:9}
I(t_{n+1})=αI(t_n)+\sum_{j=1}^nw_is_i(t-Δt)
\end{equation}
by writing \(α≔e^{-τ_{α}}\). We similarly obtain
\begin{equation}\label{eq:11}
U(t_{n+1})=βU(t_n)+I(t)+b-ϑs(t)
\end{equation}
by using the first-order exponential integrator method, defining \(β≔e^{-τ_{β}}\) and absorbing \(\frac{1-β}{τ_{β}}\) into \(I(0)\), \((w_i)_{i∈[n]}\), \(b\) and \(ϑ\).
Note that we have \(α,β∈[0,1]\) by construction.

\begin{figure}[h]
  \centering
  \input{src/figures/ch02-network-layout.tex}
  \caption{High-level network layout}
  \label{fig:network-layout}
\end{figure}

%DONE: motivate in def. why α,β∈[0,1]
We will now arrange those neurons into layers like shown in~\cref{fig:network-layout}, such that neurons are only connected to neurons from the previous layer or their own layer. Of course, since we allow recurrent connections we can always just merge all layers into the first one, but the layers do give us more control over the network.
For connections between different layers, we can use \(Δt=0\) in~\eqref{eq:9} since there can be no interdependent connections between neurons in different layers. For connections between neurons in the same layer we use \(Δt>0\) to prevent those interdependencies. Since we have previously chosen \(h=1\), it is natural to further choose \(Δt=1\).

%MAYBE: define „spike train“
Since we want to work with arbitrary data and not just spike trains, we further need to encode our data to spikes trains and decode it from spike trains. Like~\cite{nguyen2025timespikeunderstandingrepresentational} we choose a simple direct encoding and membrane potential outputs.
% DONE: why direct encoding, membrane potential output

\subsection{Definitions}
% DONE: graphic for whole network
Our type of SNN should be thought of as a composition of an initial input layer, a number of hidden spiking layers with internal state and an affine-linear layer mapping spikes activations over time, so called spike trains, to the value of the output layer.

We first define the structure of the hidden layers:

% CANCELLED: graphic for single neuron/single layer
\begin{definition}
  The \textbf{input vector} \(i^{[l]}(t)∈\{0,1\}^{n_l}\), the \textbf{spike vector} \(s^{[l]}(t)∈\{0,1\}^{n_l}\), the \textbf{pre-spike membrane potential vector} \(p^{[l]}(t)\) and the \textbf{post-spike membrane potential vector} \(u^{[l]}(t)\) of a hidden layer \(λ=(W^{[l]},b^{[l]},u^{[l]}(0),i^{[l]}(0),α^{[l]},β^{[l]},ϑ^{[l]})\) with index \(l∈[L]\), are recursively defined as
  \begin{align}
    i^{[l]}(t) & ≔ α^{[l]}i^{[l]}(t-1)+W^{[l]}s^{[l-1]}(t)+V^{[l]}s^{[l]}(t-1), \\
    p^{[l]}(t) & ≔ β^{[l]}u^{[l]}(t-1)+i^{[l]}(t)+b^{[l]}, \\
    s^{[l]}(t) & ≔ H(p^{[l]}(t)-ϑ\oneV{n_l}), \\
    u^{[l]}(t) & ≔ p^{[l]}(t)-ϑs^{[l]}(t),
  \end{align}
  with \(∀_{l∈[L]}s^{[l]}(0)=0\) and given
  \begin{enumerate}
    \item[•] \textbf{initial membrane potential}: \(u^{[l]}(0)∈ℝ^{n_l}\),
    \item[•] \textbf{initial input}: \(i^{[l]}(0)∈ℝ^{n_l}\),
    \item[•] \textbf{weight matrices}: \(W^{[l]}∈ℝ^{n_l×n_{l-1}}\), \(V^{[l]}∈ℝ^{n_l×n_l}\),
    \item[•] \textbf{bias vectors}: \(b^{[l]}∈ℝ^{n_l}\),
    \item[•] \textbf{leaky terms}: \(α^{[l]},β^{[l]}∈[0,1]\),
    \item[•] \textbf{threshold}: \(ϑ^{[l]}∈(0,∞)\),
  \end{enumerate}
  where \(H≔𝟙_{[0,∞)}\) is a step function, \(T∈ℕ\) is the number of \textbf{simulated time steps} and \(L∈ℕ\) the total \textbf{number of hidden layers}.
\end{definition}

\begin{remark}
  While it is suppressed in the notation, \(i^{[l]}\), \(p^{[l]}\), \(s^{[l]}\) and \(u^{[l]}\) clearly not only depend on \(t\), but by recursion also on \(s^{[0]}\). Further, \(s^{[l]}\) can be represented as an element of \(\{0,1\}^{n_l×T}\), which will become useful later to quantify over spike trains. In particular, we will write \(σ(t)\) for \(σ∈\{0,1\}^{n_l×T}\) and \(1∈[T]\) to mean the \(t\)-th column vector of \(σ\).
\end{remark}

We further define recurrent d.t. LIF-SNN and the function the network realizes:
\begin{definition}
  A \textbf{recurrent discrete-time LIF-SNN}, also called \rdtlifsnn, of \textbf{depth} \(L\) with \textbf{layer-widths} \((n_0,…,n_{L+1})\) and \(T∈ℕ\) time-steps is given by
  \[ Φ≔((W^{[l]},b^{[l]},V^{[l]},u^{[l]}(0),i^{[l]}(0),α^{[l]},β^{[l]},ϑ^{[l]})_{l∈[L]},T,(E,D)) \]
  where the \textbf{input encoder} \(E:ℝ^{n_0}→ℝ^{n_0×T}\) maps a vector \(x∈ℝ^{n_0}\) to a corresponding first layer spike activation \(∀_{t∈[T]}s^{[0]}(t)=E(x)(t)\) and the \textbf{output decoder} \(D:\{0,1\}^{n_L×T}→ℝ^{n_{L+1}}\) maps the spike activations of the last hidden layer to real values.
\end{definition}
% DONE: remark about spike trains being equiv. to \{0,1\}^…
% DONE: replace „recursive SNN“ etc. by “recurrent SNN”

% DONE: remark about layers being kinda useless
% DONE: remark comparison with previous model
% CANCELLED: layers mit kursiv?

\begin{definition}
  A recurrent discrete-time LIF-SNN \(ϕ\) \textbf{realizes} the function \(R(Φ):ℝ^{n_0}→ℝ^{n_{L+1}}\):
  \[ R(Φ)(x)=D((s^{[L]}(t))_{t∈[T]})\quad \text{with }s^{[0]}≔E(x)\]
\end{definition}

\begin{definition}
  A recurrent discrete-time LIF-SNN employs \textbf{direct encoding} if we have
  \[ ∀_{t∈[T]}E(x)(t)=x \]
  for the input encoder and has \textbf{membrane potential outputs} if the output decoder can be written as
  \[ D((s(t))_{t∈[T]})=\sum_{t=1}^Ta_t(W^{[L+1]}s^{[L]}(t)+b^{[L+1]}) \]
  for some \((a_t)_{t∈[T]}∈ℝ^T\), \(b^{[L+1]}∈ℝ^{n_{L+1}}\) and \(W^{[L+1]}∈ℝ^{n_{L+1}×n_L}\).
\end{definition}
\begin{remark}
  We will only consider recurrent discrete-time LIF-SNN with direct encoding and membrane potential outputs. In fact, we will use “recurrent discrete-time LIF-SNN” to mean “\rdtlifsnn with direct encoding and membrane potential”.
\end{remark}

\begin{remark}
  Our definition of \rdtlifsnn breaks now down to the definition \dtlifsnn of~\cite{nguyen2025timespikeunderstandingrepresentational} if we require \(α^{[l]}≔0\) and \(V^{[l]}\) for all layers \(l∈[L]\). So \rdtlifsnns can have recursive dependencies inside of the layers and decaying input vectors in contrast to the more simpler \dtlifsnns.
\end{remark}

Let us now take a look at some simple examples of \rdtlifsnn:

\begin{example}\label{ex:ch02-forward-spikes}
  Let \(T,L∈ℕ\). Then there exists a \rdtlifsnn with \(∀_{t∈[T]}s^{[L]}(t)=s^{[0]}(t)\) for any \(s^{[0]}∈\{0,1\}^{n_0×T}\).

  Let us use constant width \(n_l=n\), weights \(W^{[l]}=\idM{n}\), \(V^{[l]}=\zeroV{n×n}\), biases \(b^{[l]}=0\), initial input \(i^{[l]}(0)=0\), initial membrane potential \(u^{[l]}(0)=0\), leaky terms  \(α^{[l]}=β^{[l]}=0\) and threshold \(ϑ^{[l]}=1\) for all \(l∈[L]\).

  It follows from the definitions that \(i^{[l]}(t)=s^{[l-1]}(t)\) and therefore further
  \[ s^{[l]}(t) = H(s^{[l-1]}(t)-\oneV{n_l}) = s^{[l-1]}(t). \]
  So by induction we indeed get \(∀_{t∈[T]}s^{[L]}(t)=s^{[0]}(t)\).
\end{example}

\begin{example}
  Let \(T,L∈ℕ\). Then there exists a \rdtlifsnn with \(∀_{t∈T,i∈[n]}s^{[L]}_i(t)=1⇔∃_{t'∈[t-1]}(s^{[0]}_i(t'))=1\) for any \(s^{[0]}∈\{0,1\}^{n_0×T}\), i.e. an output neuron switches on once the corresponding input neurons fires.

  Let us use constant width \(n_l=n\), weights \(W^{[l]}=\idM{n}\), \(V^{[l]}=\idM{n}\), biases \(b^{[l]}=0\), initial input \(i^{[l]}(0)=0\), initial membrane potential \(u^{[l]}(0)=0\), leaky terms \(α^{[l]}=0\), \(β^{[l]}=0\) and threshold \(ϑ^{[l]}=1\) for all \(l∈[L]\).

  We then get by definition
  \[i^{[l]}(t)=s^{[l-1]}(t)+s^{[l]}(t-1)\]
  and therefore
  \[ s^{[l]}(t) = H(s^{[l-1]}(t)+s^{[l]}(t-1)-\oneV{n_l}). \]
  So if and only if \(s^{[l-1]}_i(t)=1\) or \(s^{[l]}_i(t-1)=1\), then \(s^{[l]}_i(t)=1\). We therefore have \(∀_{t∈T}s^{[l]}_i(t)=1⇔∃_{t'∈[t]}(s^{[l-1]}_i(t'))=1\) for all \(i∈[n],l∈[L]\). Thus \(∀_{t∈T,i∈[n]}s^{[L]}_i(t)=1⇔∃_{t'∈[t-1]}(s^{[0]}_i(t'))=1\) by induction.

  There also is a \dtlifsnn construction achieving the same behavior: We use the same parameters as before, but with \(V^{[l]}=\zeroV{n×n}\), \(W^{[l]}=T·\idM{n}\) and \(β^{[l]}=1\) for all layers \(l∈[L]\).

  We then get by definition
  \[i^{[l]}(t)=Ts^{[l-1]}(t)\]
  and therefore
  \begin{align*}
    s^{[l]}(t) & = H(p^{[l]}(t)-\oneV{n_l}) \\
    p^{[l]}(t) & =   p^{[l]}(t-1)+Ts^{[l-1]}(t)-s^{[l]}(t-1)
  \end{align*}
  By induction over \(t\) (see also~\cref{lem:non-recursive-defs}) we obtain 
  \[ p^{[l]}(t) = \sum_{k=1}^t\left(Ts^{[l-1]}(k)-s^{[l]}(k-1)\right) \]
  Let now \(i∈[n]\). Since \(s^{[l]}(0)=0\), we have \(\sum_{k=1}^ts^{[l]}_i(k-1)≤t-1\). Now if and only if there is any \(t_0∈[T]\) with \(s^{[l-1]}_i(t_0)=1\), we get
  \[p_i^{[l]}(t')=T\sum_{k=1}^{t'}s^{[l-1]}_i(k)-\sum_{k=1}^{t'}s^{[l]}_i(k-1)≥1\]
  for \(t'≥t_0\) and hence \(s_i^{[l]}(t')=1\).
  Just as before we now get the required property for the whole network by induction over the layers.
\end{example}

For a clearer construction of our networks we will additionally define neurons in our network:

\begin{definition}
  The \textbf{\(i\)-th neuron} of a hidden layer \(λ=(W^{[l]},b^{[l]},V^{[l]},u^{[l]}(0),i^{[l]}(0),α^{[l]},β^{[l]},ϑ^{[l]})\) of a \rdtlifsnn is a tuple \((w,b,v,u_0,i_0)\) with \(w∈ℝ^{n_{l-1}}\), \(v∈ℝ^{n_l}\) and \(b,u_0,i_0∈ℝ\), such that \(b\), \(u_0\), \(i_0\) are the \(i\)-th component of \(b^{[l]}\), \(u^{[l]}(0)\), \(i^{[l]}(0)\) respectively and \(w\), \(v\) are the \(i\)-th row vector of \(W^{[l]}\), \(V^{[l]}\) respectively.
\end{definition}

\subsection{Basic properties}

In the following we present some technical but helpful notations and lemmas for writing proofs about SNN. Of particular importance are \cref{def:non-recursive-defs} and \cref{lem:non-recursive-defs}, which introduce non-recursive formulas for the defining equations of \rdtlifsnns. 
%DONE: fix references to lem:non-recursive-defs

\begin{definition}\label{def:non-recursive-defs}
  Let \(t∈[T]\), \(l∈[L]\) and spike train families \(σ=(σ^{[l']})_{l'∈[l]_0}\), \(σ'=(σ'^{[l']})_{l'∈[l]_0}\) be given, such that \(∀_{l∈[l-1]_0}σ^{[l']}∈\{0,1\}^{n_{l'}×t}\) and \(σ^{[l]}∈\{0,1\}^{n_l×t}\) as well as \(∀_{l'∈[l]_0}σ'^{[l']}∈\{0,1\}^{n_{l'}×t}\), i.e. \(σ,σ'\) have to be chosen such that the terms in the following definitions are well-defined. We define
  \begin{align}
   i^{[l]}(t;σ)&≔(α^{[l]})^ti^{[l]}(0)+\sum_{k=1}^t(α^{[l]})^{t-k}\left(W^{[l]}σ^{[l-1]}(k)+V^{[l]}σ^{[l]}(k-1)\right), \\
   p^{[l]}(t;σ) &≔ (β^{[l]})^tu^{[l]}(0)+\sum_{k=1}^t(β^{[l]})^{t-k}\left(i^{[l]}(k;σ)+b^{[l]}\right)-ϑ\sum_{k=1}^{t-1}(β^{[l]})^{t-k}σ^{[l]}(k),\\
   s^{[l]}(t;σ) & ≔ H(p^{[l]}(t;σ)-ϑ\oneV{n_l}), \\
   u^{[l]}(t;σ') &≔ (β^{[l]})^tu^{[l]}(0)+\sum_{k=1}^t(β^{[l]})^{t-k}\left(i^{[l]}(k;σ')+b^{[l]}-ϑσ'^{[l]}(k)\right).
  \end{align}
\end{definition}

\begin{remark}
  The spike train families \(σ\), \(σ'\) in \cref{def:non-recursive-defs} are chosen such that they only include the data that is actually needed in the definition of \(i^{[l]},p^{[l]},s^{[l]},u^{[l]}\). This is also the reason why we have to use \(σ'\) for \(u^{[l]}\); its definition requires in contrast to the definitions of \(i^{[l]},p^{[l]},s^{[l]}\) the newest spikes from the current layer layer.

  We will also allow using \(i^{[l]},p^{[l]},s^{[l]},u^{[l]}\) with extensions of the by the definition necessary spike train families, in particular we will use the functions with “complete” spike train families \((σ^{[l]})_{l∈[L]}\) with \(σ^{[l]}∈\{0,1\}^{n_l×T}\).
\end{remark}

The notation for the previous definitions is justified due to

%DONE: “Let \(s^{[0]}∈\{0,1\}^{n_0×T}\) be arbitrary.” ?
\begin{lemma}\label{lem:non-recursive-defs} %DONE: introduce notation for s, σ, s'
  The non-recursive formulas from~\cref{def:non-recursive-defs} are equivalent to the recursive definitions for \(l∈[L]\), \(t∈[T]\) assuming previous spikes are equal: \(∀_{l'∈[l-1]_0}σ^{[l']}=s^{[l']}\) and \(∀_{t'∈[t-1]}σ^{[l]}(t')=s^{[l]}(t')\) as well as \(∀_{l'∈[l]_0}σ'^{[l']}=s^{[l']}\).
\end{lemma}

%DONE: introduce convention regarding spike trains function or vector/matrix
\begin{proof}
  We first proof \(∀_{t∈[T]}i^{[l]}(t;σ)=i^{[l]}(t)\) and \(∀_{t∈[T]}u^{[l]}(t;σ)=u^{[l]}(t)\) for \(σ^{[l]}=s^{[l]}\) by induction: Let \(t=1\). We have
  \begin{align*}
   i^{[l]}(1;σ)&=α^{[l]}i^{[l]}(0)+W^{[l]}σ^{[l-1]}(1)+V^{[l]}σ^{[l]}(0) \\
                         &=α^{[l]}i^{[l]}(0)+W^{[l]}s^{[l-1]}(1)+V^{[l]}s^{[l]}(0) \\
                         &=i^{[l]}(1)
  \end{align*}
  and
  \begin{align*}
   u^{[l]}(1;σ') &= β^{[l]}u^{[l]}(0)+i^{[l]}(1;σ')+b^{[l]}-ϑσ'^{[l]}(1) \\
                            &= β^{[l]}u^{[l]}(0)+i^{[l]}(1)+b^{[l]}-ϑs^{[l]}(1) \\
                            &= p^{[l]}(1)-ϑs^{[l]}(1) \\
                            &= u^{[l]}(1)
  \end{align*}
  by using the definitions and using our assumption \(σ^{[l]}=s^{[l]}\). We have \(i^{[l]}(1;σ')=i^{[l]}(1)\), since \(σ'\) is an “extension” of \(σ\).

  Let further \(t>1\). We may assume \(i^{[l]}(t-1;σ)=i^{[l]}(t-1)\) and \(u^{[l]}(t-1;σ')=u^{[l]}(t-1)\) and obtain
  \begin{align*}
   i^{[l]}(t;σ)&=(α^{[l]})^ti^{[l]}(0)+\sum_{k=1}^t(α^{[l]})^{t-k}\left(W^{[l]}σ^{[l-1]}(k)+V^{[l]}σ^{[l]}(k-1)\right) \\
                           &=α^{[l]}i^{[l]}(t-1;σ)+\left(W^{[l]}σ^{[l-1]}(t)+V^{[l]}σ^{[l]}(t-1)\right) \\
                           &=α^{[l]}i^{[l]}(t-1)+W^{[l]}s^{[l-1]}(t)+V^{[l]}s^{[l]}(t-1) \\
                           &=i^{[l]}(t)
  \end{align*}
  and similarly
  \begin{align*}
   u^{[l]}(t;σ') &= (β^{[l]})^tu^{[l]}(0)+\sum_{k=1}^t(β^{[l]})^{t-k}\left(i^{[l]}(k;σ')+b^{[l]}-ϑσ'^{[l]}(k)\right) \\
                            &= β^{[l]}u^{[l]}(t-1;σ')+\left(i^{[l]}(t;σ')+b^{[l]}-ϑσ'^{[l]}(t)\right) \\
                            &= β^{[l]}u^{[l]}(t-1)+i^{[l]}(t)+b^{[l]}-ϑs^{[l]}(t) \\
                            &= p^{[l]}(t)-ϑs^{[l]}(t) \\
                            &= u^{[l]}(t).
  \end{align*}
  By substituting \(u^{[l]}\) in \(p^{[l]}\) we further obtain
  \begin{align*}
   p^{[l]}(t;σ) &= (β^{[l]})^tu^{[l]}(0)+\sum_{k=1}^t(β^{[l]})^{t-k}\left(i^{[l]}(k;σ)+b^{[l]}\right)-ϑ\sum_{k=1}^{t-1}(β^{[l]})^{t-k}σ^{[l]}(k),\\
                            &= β^{[l]}u^{[l]}(t-1;σ)+\left(i^{[l]}(t;σ)+b^{[l]}\right)\\
                            &= β^{[l]}u^{[l]}(t-1)+i^{[l]}(t)+b^{[l]}\\
                            &= p^{[l]}(t).
  \end{align*}
  By using the above we obtain
  \[ s^{[l]}(t;σ) = H(p^{[l]}(t;σ)-ϑ\oneV{n_l})=H(p^{[l]}(t)-ϑ\oneV{n_l})=s^{[l]}(t). \]
\end{proof}

\begin{lemma}\label{lem:charac-floor}
  Let \(a,x∈ℝ\), \(a≠0\) and \(b∈ℤ\). We then have \(\lfloor\tfrac{x}{a}\rfloor=b ⇔ 0≤x-ab<a\).
\end{lemma}

\begin{proof}
  \(0≤x-ab<a\) is equivalent to \(b≤\tfrac{x}{a}<b+1\), which is yet again equivalent to \(\lfloor\tfrac{x}{a} \rfloor=b\) by definition of \(\lfloor · \rfloor\).
\end{proof}

\begin{lemma}\label{lem:spike-sum}
  Let \(t_0,t_{ω}∈[T]\) and \(i∈[n_l]\) for an \(l∈[L]\) such that \(t_0≤t_{ω}\). If \(β=1\), then \(0≤u^{[l]}_i(t_{ω})<ϑ\) holds if and only if
  \begin{equation}\label{eq:10}
    \left\lfloor \frac{1}{ϑ}\left(u^{[l]}_i(t_0-1)+\sum_{t=t_0}^{t_{ω}}(i^{[l]}_i(t)+b^{[l]}_i)\right) \right\rfloor = \sum_{t=t_0}^{t_{ω}}s^{[l]}_i(t).
  \end{equation}
\end{lemma}

\begin{proof}
  We have 
  \[ u^{[l]}_i(t_{ω})=u^{[l]}_i(t_0-1)+\sum_{k=t_0}^{t_{ω}}\left(i^{[l]}_i(k)+b^{[l]}_i-ϑs^{[l]}_i(k)\right) \]
  by~\cref{lem:non-recursive-defs}. So~\eqref{eq:10} is equal to \(0≤u^{[l]}_i(t_{ω})<ϑ\) by~\cref{lem:charac-floor}.
\end{proof}

\begin{lemma}\label{lem:u-p-nonz-eqiv}
  Let \(t∈[T]\), \(l∈[L]\) and \(i∈[n_l]\). We then have \(u_i^{[l]}(t)≥0⇔p_i^{[l]}(t)≥0\).
\end{lemma}

\begin{proof}
  Let \(u_i^{[l]}(t)≥0\). Then \(p_i^{[l]}(t)=u_i^{[l]}(t)+ϑs_i^{[l]}(t)≥0\).

  If we know \(p_i^{[l]}(t)≥0\) instead, then suppose \(u_i^{[l]}(t)<0\). Since \(p_i^{[l]}(t)≠u_i^{[l]}(t)\), \(s_i^{[l]}(t)=1\) and therefore \(p_i^{[l]}(t)≥ϑ\). But this means \(u_i^{[l]}(t)=p_i^{[l]}(t)-ϑ≥0\).
\end{proof}

\begin{lemma}\label{lem:potential-through-time-plus}
  Let \(t_0,t_{ω}∈[T]\) and \(i∈[n_l]\) for an \(l∈[L]\) such that \(t_0≤t_{ω}\). If \(∀_{t∈\{t_0+1…t_{ω}\}}i^{[l]}_i(t)+b^{[l]}_i≥0\) and \(u^{[l]}_i(t_0)≥0\), then \(u^{[l]}_i(t_ω)≥0\).
\end{lemma}

\begin{proof}
  Suppose there is a \(t\), \(t_0≤t≤t_{ω}\) with \(u^{[l]}_i(t)<0\). W.l.o.g.\ we can assume \(t\) to be minimal. Clearly \(t≠t_0\), since this contradicts our assumption. So we have \(u^{[l]}_i(t-1)≥0\) and \(i^{[l]}_i(t)+b^{[l]}_i≥0\). So from
  \[0>u^{[l]}_i(t)=p^{[l]}_i(t)-ϑs^{[l]}(t)=β^{[l]}u^{[l]}_i(t-1)+β(i^{[l]}_i(t)+b^{[l]}_i)-ϑs^{[l]}_i(t).\]
  we conclude \(s^{[l]}_i(t)=1\) and \(p^{[l]}_i(t)≥ϑ\). But this means \(u^{[l]}_i(t)≥0\) by~\cref{lem:u-p-nonz-eqiv}.
\end{proof}

\begin{lemma}\label{lem:cont-bar}
  Let \(t_0,t_{ω}∈[T]\) and \(i∈[n_l]\) for an \(l∈[L]\) such that \(t_0≤t_{ω}\). If \(∀_{t∈\rangeI{t_0}{t_{ω}}}i^{[l]}_i(t)+b^{[l]}_i≤ϑ\) and \(u^{[l]}_i(t_0-1)<ϑ\), then
  \begin{equation}\label{eq:8}
    ∀_{t∈\rangeI{t_0-1}{t_{ω}}}u^{[l]}_i(t)<ϑ.
  \end{equation}
\end{lemma}

\begin{proof}
  We proof~\eqref{eq:8} by induction over \(t∈\rangeI{t_0-1}{t_{ω}}\). The base case is given by assumption. Let further \(t∈\{t_0…t_{ω}\}\). By definition of \(p^{[l]}\), the given assumptions and the induction hypothesis we get
  \[ p^{[l]}_i(t)=β^{[l]}u^{[l]}_i(t-1)+i^{[l]}_i(t)+b^{[l]}_i ≤ u^{[l]}_i(t-1)+i^{[l]}_i(t)+b^{[l]}_i<2ϑ  \]
  We further get \(u^{[l]}_i(t)<ϑ\) by definition of \(u^{[l]}\) and \(s^{[l]}\).
\end{proof}


\begin{lemma}\label{lem:potential-through-time-minus}
  Let \(t_0,t_{ω}∈[T]\) and \(i∈[n_l]\) for an \(l∈[L]\) such that \(t_0≤t_{ω}\). If \(∀_{t∈\{t_0+1…t_{ω}\}}i^{[l]}_i(t)+b^{[l]}_i≤0\) and \(u^{[l]}_i(t_ω)≥ϑ\), then \(∀_{t∈\{t_0…t_{ω}\}}u^{[l]}_i(t)≥ϑ\).
\end{lemma}

\begin{proof}
  Suppose there is a \(t\), \(t_0≤t≤t_{ω}\), with \(u^{[l]}_i(t)<ϑ\). Let \(t\) be maximal with this property. Clearly \(t≠t_{ω}\) by assumption. So we have a contradiction by
  \[ ϑ≤u^{[l]}_i(t+1)-β^{[l]}(i^{[l]}_i(t+1)+b^{[l]}_i)+ϑs^{[l]}_i(t)=u^{[l]}_i(t). \]
\end{proof}

%DONE: i vs. j indices

\begin{lemma}\label{lem:pos-input-pos-res-pos}
  Let \(t_0,t_{ω}∈[T]\) and \(i∈[n_l]\) for an \(l∈[L]\) such that \(t_0≤t_{ω}\). If both
  \begin{equation}\label{eq:5}
    0≤(β^{[l]})^{t_{ω}-t_0}u^{[l]}_i(t_0)+\sum_{t=t_0+1}^{t_ω}(β^{[l]})^{t_{ω}-t}\left(i^{[l]}_i(t)+b^{[l]}_i\right)
  \end{equation}
  and \(∀_{t∈\rangeI{t_0+1}{t_{ω}}}s^{[l]}_i(t)=0\) or \(∀_{t∈\rangeI{t'+1}{t_{ω}}}i^{[l]}_i(t)+b^{[l]}_i≥0\), where \(t'\) is the maximal time \(≤t_{ω}\) such that \(s^{[l]}_i(t')=1\), then \(u^{[l]}_i(t_{ω})≥0\).
\end{lemma}

\begin{proof}
  If \(∀_{t∈\{t_0…t_{ω}\}}s^{[l]}_i(t)=0\), we get by assumption
  \begin{equation*}
    u^{[l]}_i(t_{ω})=u^{[l]}_i(t_{ω})+\sum_{t=t_0+1}^{t_{ω}}(β^{[l]})^{t_{ω}-t}s^{[l]}_i(t)=(β^{[l]})^{t_{ω}-t_0}u^{[l]}_i(t_0)+\sum_{t=t_0+1}^{t_ω}(β^{[l]})^{t_{ω}-t}\left(i^{[l]}_i(t)+b^{[l]}_i\right)≥0.
  \end{equation*}
  Is there on the other hand a \(t'\) with \(s^{[l]}_i(t')=1\), then let \(t'\) be maximal \(≤t_{ω}\). By assumption we now have \(∀_{t∈\{t'+1…t_{ω}\}}i^{[l]}_i(t)+b^{[l]}_i≥0\). Since \(s^{[l]}_i(t')=1\) we further have \(p^{[l]}_i(t')≥ϑ\), so by~\autoref{lem:u-p-nonz-eqiv} \(u^{[l]}_i(t')≥0\) such that we can conclude by~\autoref{lem:potential-through-time-plus}.
\end{proof}
%MAYBE: remark why more general false
%DONE: β=1 oder nicht in Formeln nicht vergessen

The following two propositions show that a neuron in a \rdtlifsnn has an internal “linear” structure.

%DONE: comment about linear structure
\begin{proposition}\label{lem:sum-spikes-slowly-through-time}
  Let \(β=1\), \(t_0,t_{ω}∈[T]\) and \(i∈[n_l]\) for an \(l∈[L]\) such that \(t_0≤t_{ω}\). Suppose \(u^{[l]}_i(t_0-1)<ϑ\) and further \(∀_{t∈\rangeI{t_0}{t_{ω}}}i^{[l]}_i(t)+b^{[l]}_i≤ϑ\). If moreover both
  \[ 0≤u^{[l]}_i(t_0-1)+\sum_{t=t_0}^{t_ω}\left(i^{[l]}_i(t)+b^{[l]}_i\right) \]
  and \(∀_{t∈\rangeI{t_0}{t_{ω}}}s^{[l]}_i(t)=0\) or \(∀_{t∈\rangeI{t'+1}{t_{ω}}}i^{[l]}_i(t)+b^{[l]}_i≥0\), where \(t'\) is the last time \(≤t_{ω}\) such that \(s^{[l]}_i(t')=1\), then
  \[ \sum_{t=t_0}^{t_{ω}}s^{[l]}_i(t)=\left\lfloor \frac{1}{ϑ}\left(u^{[l]}_i(t_{0}-1)+\sum_{t=t_0}^{t_m}\left(i^{[l]}_i(t)+b^{[l]}_i\right)\right) \right\rfloor. \]
\end{proposition}

\begin{proof}
  It suffices to show \(0≤u^{[l]}_i(t_{ω})<ϑ\) due to~\cref{lem:spike-sum}.
  We get \(0≤u^{[l]}_i(t_{ω})\) due to~\cref{lem:pos-input-pos-res-pos} and \(u^{[l]}_i(t_{ω})<ϑ\) due to~\cref{lem:cont-bar}.

\end{proof}

\begin{proposition}\label{lem:sum-spikes-decaying}
  Let \(β=1\), \(t_0,t_m,t_{ω}∈[T]\) and \(i∈[n_l]\) for an \(l∈[L]\) such that \(t_0≤t_m≤t_{ω}\). If \(∀_{t∈\rangeI{t_m+1}{t_{ω}}}i^{[l]}_i(t)+b^{[l]}_i=0\), \(u^{[l]}_i(t_m)≥0\) and
  \[ 0≤u^{[l]}_i(t_0-1)+\sum_{t=t_0}^{t_m}\left(i^{[l]}_i(t)+b^{[l]}_i\right)≤ϑ(t_ω-t_m+1), \]
  then
  \[ \sum_{t=t_0}^{t_{ω}}s^{[l]}_i(t)=\left\lfloor \frac{1}{ϑ}\left(u^{[l]}_i(t_{0}-1)+\sum_{t=t_0}^{t_m}\left(i^{[l]}_i(t)+b^{[l]}_i\right)\right) \right\rfloor. \]
\end{proposition}

\begin{proof}
  It suffices to show \(0≤u^{[l]}_i(t_{ω})<ϑ\) due to~\cref{lem:spike-sum}.
  We get \(u^{[l]}_i(t_{ω})≥0\) from~\autoref{lem:potential-through-time-plus} using our assumptions, in particular \(u^{[l]}_i(t_m)≥0\).
  Furthermore, \(u^{[l]}_i(t_{ω})<ϑ\); otherwise, if \(u^{[l]}_i(t_{ω})≥ϑ\), then we had \(∀_{t∈\{t_m…t_{ω}\}}u^{[l]}_i(t)≥ϑ\) by~\autoref{lem:potential-through-time-minus} and therefore in particular \(∀_{t∈\{t_m…t_{ω}\}}s^{[l]}_i(t)=1\), from which we get
  \begin{align*}
   u^{[l]}_i(t_{ω}) &= u^{[l]}_i(t_0-1)+\sum_{k=t_0}^{t_{ω}}\left(i^{[l]}(k)+b^{[l]}-ϑs^{[l]}(k)\right)\\
                   &= u^{[l]}_i(t_0-1)+\sum_{k=t_0}^{t_m}\left(i^{[l]}(k)+b^{[l]}\right)-ϑ\sum_{k=t_0}^{t_{ω}}s^{[l]}(k) \\
                 &≤ ϑ(t_ω-t_m+1)-ϑ(t_ω-t_m+1)-ϑ\sum_{k=t_0}^{t_{m}-1}s^{[l]}(k) \\
                 &≤ 0
  \end{align*}
  contradicting \(u^{[l]}_i(t_{ω})≥ϑ\).
\end{proof}
