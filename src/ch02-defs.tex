\section{Motivation and definition of \rdtlifsnn}
\label{ch:defs}

\subsection{Motivation}
Like~\cite{nguyen2025timespikeunderstandingrepresentational} we want to model the brain more closely. To this aim we will use a slightly more complex model than~\cite{nguyen2025timespikeunderstandingrepresentational} did.

% TODO: link paper for equations
We have equations
\begin{align*}
...
\begin{align*}
  &e^{-τ_{α}t_n}\left(I(t_{n+1})-e^{-τ_{α}h}I(t_n)\right) \\
  &=\int_{t_n}^{t_{n+1}}\frac{d}{dt}\left(e^{-τ_{α}t}I(t)\right)dt \\
  &=\int_{t_n}^{t_{n+1}}e^{-τ_{α}t}(I'(t)-τ_αI(t))dt \\
  &=\int_{t_n}^{t_{n+1}}e^{-τ_{α}t}\sum_{j=1}^nw_js_j(t-Δt)dt
\end{align*}
...
\begin{align*}
 ...
 &=\frac{1}{τ_{α}}e^{-τ_{α}t_n}(1-e^{-τ_{α}h})\sum_{j=1}^nw_js_j(t-Δt)
\end{align*}
So we get
\[ I(t_{n+1})=e^{-τ_{α}h}I(t_n)+\frac{1}{τ_{α}}(1-e^{-τ_{α}h})\sum_{j=1}^nw_js_j(t-Δt) \]
Let us define \(h≔1\) and \(α≔e^{-τ_{α}}\). We get
\[ I(t_{n+1})=αI(t_n)+\sum_{j=1}^nw_js_j(t-Δt) \]
By absorbing \(\frac{1-α}{τ_{α}}\) into the weights \((w_j)_{j∈[n]}\). We obtain similarly
...
\begin{notation}
  We will use \(⟦x,y⟧\), \(⟦x,y⦆\) with \(x,y∈ℝ^n\) to write half-open cuboids \(\prod_{i=1}^n[x_i,y_i)\), and closed cuboids \(\prod_{i=1}^n[x_i,y_i]\) respectively. We further take \([x,y)\) to be empty for \(x,y∈ℝ\) with \(x≥y\), and \([-∞,x)]\) to mean \((-∞,x)\).
\end{notation}
...
\begin{proof}
  ...
  due to~\cref{lem:non-recursive-defs}. From which we immediately obtain
  \[ \left( u^{[l]}_j(t_0-1)+\sum_{t=t_0}^{t_{ω}}(i^{[l]}_j(t)+b^{[l]}_j) \right) - ϑ\sum_{t=t_0}^{t_{ω}}s^{[l]}_j(t) <ϑ \]
  ...
  by~\cref{lem:non-recursive-defs}. By assumption and choice of \(t'\) we can conclude
