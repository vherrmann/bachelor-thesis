...
\begin{proof}\phantom{}
  ...
  Is on the other hand \(ω^{†}(s)=0\), then there is a sequence \((t_n)_{n∈ℕ}\) with \(t_n→0\), such that \(ω(t_n)>s\) by definition of \(ω^{†}\). But since \(ω(t_n)→0\) by definition of \(ω\), we get \(s=0\).
  \end{enumerate}
\end{proof}
...
\begin{theorem}\label{thm:approx-snn}
  Let a \(f∈𝒞^0(U,ℝ^n)\) be a continuous function, such that \(f\mid_{U°}∈𝒞^1(U°,ℝ^m)\) is continuously differentiable with bounded differential, i.e. \(\norm{d(f|_{U°})}_{∞,2}<∞\). Let further \(∅≠Ω⊂U\) be an arbitrary non-empty subset and \(C⊂ℝ^n\) an half-open cube such that \(Ω⊂C\) and \((Ω+B_{ρ,2}(0))∩C°⊂U°\) for a \(ρ>0\). %DONE: graphic of Ω, U, C, N_ρ(Ω)

  For all \(ε,μ,ν>0\), \(ε=μ+ν\), there exists a \rdtlifsnn \(Φ\) with \(L=2\) and
  \begin{align*}
   T   &= (K(μ)+1)T_r(ν)+2 \\
   n_1 &= n+1 \\
   n_2 &= K(μ)^n(n+1)+3
  ...
  \[ \norm{R(Φ)|_Ω-f}_{∞,2}≤ε.\]
  Where we use 
  \begin{align*} %DONE: add ρ to later statements, replace d(f|_{C°}) by d(f|_{U°})
   T_r&≔T_r(ν)≔\max\left(2,\left\lceil \sqrt{n}\frac{\operatorname{diam}_{∞}(C)}{K}\frac{\norm{d(f|_{U°})}_{∞,2}}{ν} \right\rceil\right), \\ %DONE: fix T_{r} later
   K&≔K(μ)≔ \min_{\substack{ξ,θ>0\\ξθ=μ}}\left\{\left\lceil \frac{\operatorname{diam}_∞(C)}{\frac{2}{\sqrt{n}}\min(ω^{†}(ξ) ,θ,\frac{ρ}{2})} \right\rceil\right\}.
  \end{align*} %DONE: if ω^{†}=∞, then still K≠0?
  Here \(ω^{†}\) is the generalized inverse of a modulus of uniform continuity with regard to \(\norm{·}_2\) of the total derivative \(d(f|_{U°})\). \(d(f|_{U°})\) is uniformly continuous since it is bounded.
  Since \(ξ≠0\), we have \(ω^{†}(ξ)>0\) by~\cref{lem:inverse-cont-mod}. Further there are \(ξ,θ\) such that the minimum in the definition of \(K^n\) is obtained, since we the minimum is taken over the set of natural numbers, so \(K\) is well-defined. Further \(\min(ω^{†}(ξ),θ)<∞\) as well as \(\operatorname{diam}_{∞}(C)≠0\), since \(Ω⊂C\) and \(Ω≠∅\). We therefore have \(K≠0\), such that \(T_r\) is well-defined.
\end{theorem}

\begin{remark}\label{rem:ch03-use-cube}
  Let \(f:C→ℝ^n\) be defined on a half-open cube \(C\) and \(Ω=C\). We can then choose an arbitrarily big \(ρ\), so we can drop \(\frac{ρ}{2}\) from the definition of \(K\),
  \[ K= \min_{\substack{ξ,θ>0\\ξθ=μ}}\left\{\left\lceil \frac{\operatorname{diam}_∞(C)}{\frac{2}{\sqrt{n}}\min(ω^{†}(ξ) ,θ)} \right\rceil\right\}. \]
\end{remark}

\begin{remark}\label{rem:ch03-differential-lipschitz}
  If the differential \(d(f|_{U°})\) is \(L\)-Lipschitz in addition to the conditions in~\cref{thm:approx-snn}, we can simplify the definition of \(K\). Let us choose the canonical modulus of uniform continuity \(ω(x)≔Lx\), such that \(ω^{†}(x)=\frac{1}{L}x\) (with \(\frac{1}{L}=∞\) for \(L=0\)).
  Now for \(L>0\) we have
  \[ \max_{\substack{ξ,θ>0\\ξθ=μ}}\min(ω^{†}(ξ) ,θ)=\max_{\substack{ξ,θ>0\\ξθ=μ}}\min(\frac{1}{L}ξ ,\frac{μ}{ξ})=\sqrt{\frac{μ}{L}} \]
  since \(\frac{1}{L}ξ\) is montonically increasing in \(ξ\), \(\frac{μ}{ξ}\) monotonically decreasing in \(ξ\) and \(\frac{1}{L}ξ=\frac{μ}{ξ}\) equivalent to \(ξ=\sqrt{μL}\) for \(ξ>0\).
  So we have
  \[ K= \left\lceil \frac{\operatorname{diam}_∞(C)}{\frac{2}{\sqrt{n}}\min(\sqrt{\frac{μ}{L}},\frac{ρ}{2})} \right\rceil. \]
  For \(L=0\) we have \(\min(ω^{†}(ξ) ,θ)=θ\), so \(K=1\). We therefore get
  \[ K= 1∨\left\lceil \frac{\operatorname{diam}_∞(C)}{\frac{2}{\sqrt{n}}\min(\sqrt{\frac{μ}{L}},\frac{ρ}{2})} \right\rceil \]
  for \(L≥0\) arbitrary.
\end{remark}
...
\begin{remark}
  The conditions of the theorem regarding \(U,Ω,C\) might seem unnecessarily complex at first, but they are indeed necessary to approximate some functions efficiently.
  For our construction of the \rdtlifsnn we need piece-wise affine-linear approximations of \(f\), defined on subcubes of a cube \(C⊂ℝ^n\). Suppose, the theorem just required \(f\) to be defined on that cube \(C\) instead and further \(Ω=C\).

  Now picture a function \(f\) like the one shown in~\cref{fig:ch03-spiral-staircase}. If we want to approximate the whole complete input space \(Ω\) of \(f\), we need to put a half-open square \(C\) around the arc due to our construction. To further use the theorem with \(U=Ω=C\) we would need to have differentiability on the whole of \(C°\), but \(f\) is only defined on an arc inside of \(C\).
  This problem can be handled by replacing \(f\) with an extension of \(f\), defined on all of \(ℝ^n\), but now we have the problem that we might get a very steep graph around \((-0.5,0.0)\), such that \(\norm{d(f|_{C°})}_{∞,2}\) is unnecessarily large and changes unnecessarily quick. So \(T_r\) and \(K\) would get excessively large.

  On the other hand,~\cref{thm:approx-snn} only uses the values of \(df\) on a thin region around \(Ω\) to determine how large \(K\) and \(T_r\) need to be.
\end{remark}
...
\begin{remark}
  ...
  For our construction of the SNN we now need an encompassing half-open rectangle of \(Ω\), let e.g. \(C=[0,2)^2\). Now in~\cref{lem:approx-by-lin} we will partition \(C\) into sub-cubes, such that \(f\) is almost linear on those  w%CANCELLED
...
%DONE: remark about usage with \(Ω=U=C\).
...
% \item We would like to only demand differentiability on \(Ω°\), … %CANCELLED
...
\begin{proof}
  Let \(ε>0\) with modulus \(δ(ε)\), as well as \(x∈M\) be given. Since \(f\) is uniformly continuous with modulus \(δ(ε)\), we have \(f(B_{δ,2}(x))⊂B_{ε,2}(f(x))\). Since \(f\) is in particular continuous, we get
  \[ f(\overline{B}_{δ}(x))=f(\overline{B_{δ,2}(x)})⊂\overline{f(B_{δ,2}(x))}⊂\overline{B}_{ε}(f(x)) \]
  We need to use the fact that \(M\) is a normed vector space in the first equality to get \(\overline{B}_{δ}(x)=\overline{B_{δ,2}(x)}\).
\end{proof}
\end{comment}
...
\begin{lemma}\label{lem:approx-by-lin}
  Let a \(f∈𝒞^0(U,ℝ^n)\) be a continuous function, such that \(f\mid_{U°}∈𝒞^1(U°,ℝ^m)\) is continuously differentiable with a uniform continuous differential \(d(f|_{U°})\). Let further \(∅≠Ω⊂U\) be an arbitrary non-empty subset and \(C⊂ℝ^n\) an half-open cube such that \(Ω⊂C\) and \((Ω+B_{ρ,2}(0))∩C°⊂U°\) for a \(ρ>0\).
  Let further \(K\) be defined as in~\cref{thm:approx-snn}.

  For every \(μ>0\) we can compose \(C\) into \(K^n\) half-open subcubes \((C^{(j)})_{j∈[K^n]}\) such that affine linear functions \(g^{(j)}:C^{(j)}→ℝ^m\) exist with  \(\norm{d(g^{(j)})}_{∞,2}≤\norm{d(f|_{U°})}_{∞,2}\) and \(\norm{(f-g)|_Ω}_{∞,2}<μ\), where \(g≔\sum_{i=1}^mg^{(j)}χ_{C^{(j)}}\).
\end{lemma}
...
\begin{proof}[\proofofref{lem:approx-by-lin}]
  Let \(μ>0\) be given. Let \(ω\) be a modulus of uniform continuity of \(df\mid_{U°}\).
  We will now partition \(C\) in \(K^n\) half-open subcubes with \(K\) defined as in~\cref{thm:approx-snn}.
  Let \(ξ,Θ\) be given, such that the minium in the definition of \(K\) is attained. Then the subcubes have width
  \[ w≔\frac{\operatorname{diam}_∞(C)}{K}≤\frac{2}{\sqrt{n}}\min(ω^{†}(ξ),θ,\frac{ρ}{2}). \]

  Let now \(c^{(j)}\) be the center of \(C^{(j)}\), such that in particular for all \(x∈C^{(j)}\)
  \begin{equation}\label{eq:4}
  \norm{x-c^{(j)}}_2=\sqrt{\sum_{i=1}^n\abs{(x-c^{(j)})_i}^2}≤\frac{w}{2}\sqrt{n}≤\min(ω^{†}(ξ),θ,\frac{ρ}{2}).
  \end{equation}

  Let us further define \(g^{(j)}:C^{(j)}→ℝ^m\) by
  \[ g^{(j)}≔\begin{cases} x↦f(c^{(j)})+df_{c^{(j)}}(x-c^{(j)}), & C^{(j)}∩Ω≠∅ \\ x↦0, & C^{(j)}∩Ω=∅ \\ \end{cases}. \]
  The first case is well-defined, since \(C^{(j)}∩Ω≠∅\) implies that there exists a \(x∈ℝ^n\) with \(x∈Ω\) and \(∀_{y∈C^{(j)}}\norm{x-y}_2≤ρ\) by~\eqref{eq:4}, so \(C^{(j)}⊂(Ω+\overline{B}_{ρ,2}(0))∩C\) and therefore \((C^{(j)})°⊂(Ω+\overline{B}_{ρ,2}(0))∩C°⊂U°\).
  Now by definition of \(g^{(j)}\) we already have \(\norm{d(g^{(j)})}_{∞,2}=\norm{df_{c^{(j)}}}≤\norm{d(f|_{U°})}_{∞,2}\) for \(C^{(j)}∩Ω≠∅\) and otherwise \(0=\norm{d(g^{(j)})}_{∞,2}≤\norm{d(f|_{U°})}_{∞,2}\).

  It only remains to show \(\norm{(f-g^{(j)})|_{Ω∩C^{(j)}}}_∞<μ\). This is clearly trivial for \(C^{(j)}∩Ω=∅\), so suppose \(C^{(j)}∩Ω≠∅\). Let \(x∈C^{(j)}\) and \(h(t)≔f(t(x-c^{(j)})+c^{(j)})\). We then have
  \[ h'(t)=(df_{(x-c^{(j)})t+c^{(j)}}∘d(t↦t(x-c^{(j)})+c^{(j)})_t)(1)=df_{(x-c^{(j)})t+c^{(j)}}(x-c^{(j)}). \]
  Since \((C^{(j)})°⊂U°\), we obtain by the fundamental theorem of calculus
  \begin{align*}
...
\begin{proof}[\proofofref{thm:approx-snn}.]
  ...
  \end{gather*}
  Note that \(T_2\) overlaps with \(T_1\). For ease of notation, will also use \(T_2,T_3,T_4\) as if they were numbers.
  ...
  The higher level idea is that we accumulate the state during \(T_1\), compute the subregion \(C^{(j)}\) of the input during \(T_2,…,T_4\) and flush out the position of the input inside of \(C^{(j)}\) during \(T_5\).
  %DONE:: replace j by 1 in 3.5

  The constructed first layer is only active during the first phase, \(T_1\). It is composed of \(n+1\) neurons, where the first \(n\) neuron convert the input vector regarding its position in \(C\) into spike trains. The last neuron, the “alarm clock”, shuts down the first layer after \(T_1\) ends.
  ...
  Let us now construct the second layer in the following way: For each of the \(K^n\) subcubes in \(C\) we define \(n+1\) neurons like so: Let \(C^{(j)}=⟦x^{C^{(j)}},y^{C^{(j)}}⦆\) be one such subcube with position \(q^{(j)}∈([K-1]_0)^n\) in \(C\), i.e.
  \[∀_{i∈[n]}q^{(j)}_i=Ko_i(x^{C^{(j)}}).\]
  ...
  \end{equation}
  where “the switch” is
  \[ r(q)≔e_{ω_{j(q)}}-\sum_{\substack{q'∈([K-1]_0)^n \\ q<q'}}e_{ω_{j(q')}}. \]
  with the index \(j(q)\) of the subcube at position \(q\). We further define the applied variant
  \[ r_s(q;t)≔⟨r(q),s^{[2]}(t)⟩=s^{[2]}_{ω_{j(q)}}(t)-\sum_{\substack{q'∈([K-1]_0)^n \\ q<q'}}s^{[2]}_{ω_{j(q')}}(t). \]
  ...
  \item Output decoder:
    ...
    \[ W^{[L+1]}_{k,ι_j(i)}=d(g^{(j)})_k((y^{C^{(j)}}_i-x^{C^{(j)}}_i)\frac{1}{T_r}e_i) \]
    for \(k∈[m]\), \(j∈K^n\) and \(i∈[n]\). Here \(d(g^{(j)})\) is not only the total derivative, but also the linear part of \(g^{(j)}\), i.e. \(g^{(j)}(x)=d(g^{(j)})(x-x^{C^{(j)}})+g(x^{C^{(j)}})\) for all \(x∈C^{(j)}\). We further set
  ...
  Let further \(1≤t≤T_2-1\). First notice that by induction hypothesis, we have \(∀_{i∈[n]}s^{[2]}_{ι_j(i)}(k)=0\) and \(r_s(q^{(j)};k)=0\) for \(k<t\). It follows that
  \begin{align*}
   i^{[2]}_{ι_j(i)}(k)+b^{[2]}_{ι_j(i)}&=s^{[1]}_i(k)+T(s^{[2]}_{c_1}(k-1)-2s^{[2]}_{c_2}(k-1)+r_s(q^{(j)};k-1))=s^{[1]}_i(k), \\
   i^{[2]}_{ω_j}(k)+b^{[2]}_{ω_j}&=\frac{1}{n}\sum_{i=1}^ns^{[2]}_{ι_j(i)}(k-1)-2s^{[2]}_{a_2}(k-1)+r_s(q^{(j)};k-1)=0.
  \end{align*}
  for all \(k≤t\). Thus, we further get
  ...
  \end{align*}
   Since \(\sum_{k=1}^ts^{[1]}_i(k)≤T_2-1<T-1\) and \(s^{[2]}_{c_1}(t)=χ_{\{T_2-1\}}(t)\), we in particular get \(p^{[2]}_{ι_j(i)}(t)≤0\). So we have proven \(∀_{i∈[n]}s^{[2]}_{ι_j(i)}(t)=0\), \(s^{[2]}_{ω_j}(t)=0\).
  \item Phase 2 \\
  ...
  \end{align*}
  So \(0≤p^{[2]}_{ω_j}(T_3)≤1\) and we get \(p^{[2]}_{ω_j}(T_3)=1\), as well as \(s^{[2]}_{ω_j}(T_3)=1\) exactly if \(∀_{i∈[n]}x^{C^{(j)}}_i≤x_i\), so if \(x^{C^{(j)}}≤x\).

  Let further \(i∈[n]\). We get
  \begin{align*}
  ...
  \end{align*}
  By definition of \(q\), \(q^{(j)}_i<q^{(j')}_i\) holds exactly if \(x^{C^{(j)}}_i<x^{C^{(j')}}_i\) holds for all \(i∈[n]\) and \(j,j'∈[K^n]\), so \(∀_{j,j'∈[K^n]}q^{(j)}<q^{(j')}⇔x^{C^{(j)}}<x^{C^{(j')}}\).
  We further have shown \(∀_{j'∈K^n}s^{[2]}_{ω_{j'}}(T_3)=1⇔x^{C^{(j')}}≤x\) before.
  \begin{align*}
  r_s(q^{(j)};T_3)&=s^{[2]}_{ω_j}(T_3)-\sum_{\substack{q'∈([K-1]_0)^{n} \\ q^{(j)}<q'}}s^{[2]}_{ω_{j(q')}}(T_3) \\
  ...
  \end{align*}
  And therefore for all \(k∈[m]\) %DONE: split into parts
  \begin{align*}
   W^{[L+1]}_{k,ω_j}\sum_{t=T_4}^Ts^{[2]}_{ω_j}(t) &= g_k(x^{C^{(j)}}), \\
   W^{[L+1]}_{k,ι_{j(i)}}\sum_{t=T_4}^Ts^{[2]}_{ι_{j(i)}}(t) &= \left(-KT_ro_i(x^{C^{(j)}})+\left\lfloor KT_ro_i(x)\right\rfloor\right)d(g^{(j)})_k((y^{C^{(j)}}_i-x^{C^{(j)}}_i)\tfrac{1}{T_r}e_i).
  \end{align*}
  ...
  \begin{align*}
    \sum_{t=T_4}^Ts^{[2]}_{ι_{j(i)}}(t)=0,\qquad \sum_{t=T_4}^Ts^{[2]}_{ω_j}(t)=0.
  \end{align*}
  So this group of neurons does not contribute to the output of the network:
  \begin{align*}
   W^{[L+1]}_{k,ω_j}\sum_{t=T_4}^Ts^{[2]}_{ω_j}(t) = 0,\qquad W^{[L+1]}_{k,ι_{j(i)}}\sum_{t=T_4}^Ts^{[2]}_{ι_{j(i)}}(t) = 0.
  \end{align*}
  ...
  \begin{align*}
    & g_k(x^{C^{(j)}})+\sum_{i∈[n]}\left(d(g^{(j)})_k((y^{C^{(j)}}_i-x^{C^{(j)}}_i)\frac{1}{T_r}e_i)\right)\left(-KT_ro_i(x^{C^{(j)}})+\left\lfloor KT_ro_i(x) \right\rfloor\right) \\
    &= g_k(\underset{≕x'}{\underbrace{x^{C^{(j)}}+\sum_{i∈[n]}\tfrac{y^{C^{(j)}}_i-x^{C^{(j)}}_i}{T_r}\left(-KT_ro_i(x^{C^{(j)}})+\left\lfloor KT_ro_i(x) \right\rfloor\right)e_i}})
  \end{align*}
  Now by assumption, we have
  \[T_r=\sqrt{n}\frac{y^C_i-x^C_i}{K}\frac{\norm{df}_{∞,2}}{2ν}≥\sqrt{n}(y^{C^{(j)}}_i-x^{C^{(j)}}_i)\frac{\norm{d(g^{(j)})}_{∞,2}}{2ν} \]
  for any \(i∈[n]\) and hence
  \[ ξ_i≔\frac{1}{T_r}\left(\left\lfloor KT_ro_i(x)\right\rfloor-KT_ro_i(x)\right)≤\frac{1}{\sqrt{n}(y^{C^{(j)}}_i-x^{C^{(j)}}_i)}\frac{2ν}{\norm{d(g^{(j)})}_{∞,2}}. \]
  We now get the following inequality by definition of \(ξ_i\) and \((y^{C^(j)}_i-x^{C^(j)}_i)K = (y^{C}_i-x^{C}_i)\)
  \begin{align*}
    \norm{x'-x}_2^2 &=\sum_{i∈[n]}\left(x'-x_i\right)^2 \\
    &=\sum_{i∈[n]}\left((y^{C^{(j)}}_i-x^{C^{(j)}}_i)\left(\tfrac{1}{T_r}\left\lfloor KT_ro_i(x)\right\rfloor- Ko_i(x^{C^{(j)}})\right)-(x_i-x^{C^{(j)}}_i)\right)^2 \\
    &=\sum_{i∈[n]}\left((y^{C^{(j)}}_i-x^{C^{(j)}}_i)\left(ξ_i+K\frac{x_i-x^{C^{(j)}}_i}{y^C_i-x^C_i}\right)-(x_i-x^{C^{(j)}}_i)\right)^2 \\
    &=\sum_{i∈[n]}\left(\left(ξ_i(y^{C^{(j)}}_i-x^{C^{(j)}}_i)+(x_i-x^{C^{(j)}}_i)\right)-(x_i-x^{C^{(j)}}_i)\right)^2 \\
    &=\sum_{i∈[n]}ξ_i^2(y^{C^{(j)}}_i-x^{C^{(j)}}_i)^2 \\
    &≤\frac{ν^2}{\norm{d(g^{(j)})}_{∞,2}^2}.
  \end{align*}
  So we can conclude
  \[ \norm{g(x)-R(Φ)_k(x)}_2=\norm{g(x)-g(x')}_2=\norm{d(g^{(j)})(x-x')}_2 ≤\norm{d(g^{(j)})}_{∞,2}\norm{x-x'}_2 ≤ν \]
...
Sadly the size of the network in this construction is not always smaller than the one from~\cref{thm:approx-snn-constant}. A concrete counter example is a sinus wave with high frequency and small ampitude, like \(f(x)≔\frac{\sin(nx)}{n}\) with \(n∈ℕ\) on \(C=U=[0,3π)\) and \(Ω=[0,2π]\). Since \(f'(x)=\cos(nx)\) and \(\norm{f'|_{(0,3π)}}_{∞}=1\), \(Γ≔1\) is the optimal Lipschitz-constant for \(f\).
At the same time, since \(f''(x)=n\sin(nx)\) and \(\norm{f''|_{(0,3π)}}_{∞}=n\), the biggest possible modulus of uniform continuity on \((0,3π)\) for \(f'\) is \(δ(ε)≔\frac{ε}{n}\). So we get
\[ \max_{\substack{ξ,θ>0\\ξθ=ε}}\min(δ(ξ),θ)=\max_{\substack{ξ>0}}\min(\frac{ξ}{n},\frac{ε}{ξ})=\sqrt{\frac{ε}{n}}. \]
So we get \(K(ε)≔\lfloor π\sqrt{\frac{n}{ε}} \rfloor\). We therefore get for the layer sizes:
\begin{alignat*}{2}
  & \cref{thm:approx-snn-constant} \qquad &\qquad  &\cref{thm:approx-snn} \\
  &n_1 =\left\lceil \tfrac{2π}{ε} \right\rceil+1 \qquad &\qquad &n_1 = 2 \\
  &n_2 =\left\lceil \tfrac{2π}{ε} \right\rceil \qquad &\qquad &n_2 = 2\lfloor π\sqrt{\tfrac{n}{ε}} \rfloor+3
\end{alignat*}
...
%MAYBE: burden of Γ is placed on T in our theorem

%DONE: introduce notation C^0, C^1
We generalize this observation with the following theorem.
\begin{theorem} %DONE: switch to U,Ω,C
  Let the function \(f\) and the sets \(U\), \(C\), \(Ω\) be given as inf~\cref{thm:approx-snn}. Let further \(Ω\) be compact, assume that \(f\) is \(Γ'\)-Lipschitz, and \(d(f|_{U°})\) is \(Γ\)-Lipschitz.

  We then have
  \[ \lim_{ε→0}\frac{n_2(ε)}{n_2'(ε)}=0. \]
  Where \(n_2'(ε)\) is \(n_2(ε)\) as defined in~\cref{thm:approx-snn-constant} for \(Ω\), \(f|_{Ω}\) with Lipschitz-constant \(Γ'\) and \(ε\); and where \(n_2(ε)\) is as defined in~\cref{thm:approx-snn} for sets \(U\), \(C\), \(Ω\) and function \(f\) with modulus of uniform continuity \(ω(x)≔Γx\) of \(d(f|_{U°})\) and approximation control \(μ≔\frac{ε}{2}\).
\end{theorem}
...
\begin{proof}
  ...
  \[n_2'(ε)=\max\left\{\left\lceil \frac{\operatorname{diam}_∞(Ω)}{ε}Γ' \right\rceil^n,1\right\}\]
  we have
  \[ \lim_{ε→0}n_2'(ε)ε=(\operatorname{diam}_∞(Ω)Γ')^n. \]
  We further have (compare~\cref{rem:ch03-differential-lipschitz})
  \begin{align*}
   n_2(ε)&= \left(1∨\left\lceil \frac{\operatorname{diam}_∞(C)}{\frac{2}{\sqrt{n}}\min(\sqrt{\frac{ε}{2Γ}},\frac{ρ}{2})} \right\rceil\right)(n+1)+3
  \end{align*}
  such that we get
  \[ \lim_{ε→0}n_2(ε)\sqrt{ε}=\sqrt{\frac{nΓ}{2}}\operatorname{diam}_∞(C)(n+1). \]
  We can conclude
  \[ \lim_{ε→0}\frac{n_2(ε)}{n_2'(ε)}\frac{1}{\sqrt{ε}}=\lim_{ε→0}\frac{n_2(ε)ε}{n_2'(ε)\sqrt{ε}}=\frac{\sqrt{\frac{nΓ}{2}}\operatorname{diam}_∞(C)(n+1)}{\operatorname{diam}_∞(Ω)Γ'}∈ℝ \]
  So we can deduce
  \[\lim_{ε→0}\frac{n_2(ε)}{n_2'(ε)}=0.\]
\end{proof}
...
To conclude this section, we shall give one last remark. While we used \rdtlifsnn for our construction in~\cref{thm:approx-snn}, we do think that it is not too hard too remove the interdependencies inside of the layers too obtain a \dtlifsnn by replicating layers as often as needed and connecting where necessary.
E.g.\ we can remove the interdependency of \(a_1\) on itself in the construction of~\cref{thm:approx-snn} by using \(u_0=-T_2\) and \(b=1\) to obtain the “alarm clock” behavior.
Further its connections to the other neurons in the first layer can be removed by adding an intermediate layer between first and second hidden layer with \(n\) neurons that just forward the input from their respective neuron in the first layer (see also~\cref{ex:ch02-forward-spikes}) and also take in an input from \(a_1\), compare~\cref{fig:ch03-alternative-construction}.
The reason why we have still chosen to go ahead with \rdtlifsnn in our proof is that the network complexity is smaller.

%DONE: a1

%CANCELLED: betrachtung durch kombinatorik/wahrscheinlichkeit?

%DONE: there are also constructions for \dtlifsnn that use far less neurons, general structure like…

% CANCELLED: encoding with fewer layers for on finite segments continuously differentiable functions


% DONE: wendeltreppe
% DONE: flush nach KTr (high-level erklärung verhalten)
% DONE: vergleich d.t.lif-nn, r.constr, auch ohne d.t.lifsnn möglich?
