...
\begin{proof}[\proofofref{thm:approx-snn}.]
  ...
  \begin{align*}
   i^{[1]}_i(t)=\frac{1}{y^C_i-x^C_i}s^{[0]}_i(t)-s^{[1]}_{n+1}(t-1)=\frac{1}{y^C_i-x^C_i}s^{[0]}_i(t)-s^{[1]}_{n+1}(t-1)
  \end{align*}
  So we can use~\autoref{lem:sum-spikes-over-time} for \(x^C_i≤s^{[0]}_i(t)<y_i\) and \(t≤K(ξ)T_r(θ)\) to obtain
  \begin{equation}\label{eq:3}
  \left\lfloor \frac{K(ξ)T_r(θ)}{y^C_i-x^C_i}(s^{[0]}_i(1)-x^C_i) \right\rfloor =\left\lfloor u^{[1]}_i(0)+\sum_{t=1}^{K(ξ)T_r(θ)}(i^{[1]}_i(t)+b^{[1]}_i) \right\rfloor = \sum_{t=1}^{K(ξ)T_r(θ)}s^{[1]}_i(t)
  \end{equation}
  ...
  \begin{gather*}
    ...
    u_0=-q_iT_r(θ)+1, \quad i_0=0.
  \end{gather*}
  where \(j(q)\) is the index of the subcube at position \(q\).
  We get
  \[ p^{[2]}_{ι_j(i)}(t) = -q_iT_r(θ)+1+\sum_{k=1}^ts^{[1]}_i(k)-(K(ξ)T_r(θ)+1)\sum_{k=1}^{t-1}s^{[2]}_{ι_j(i)}(k). \]
  for \(t≤K(ξ)T_r(θ)\), since as we will later show, \(s^{[1]}_{ω_j}(t)=0\) for all \(j∈[K^n]\) and \(t≤K(ξ)T_r(θ)\). We therefore have \(s^{[2]}_{ι_j(i)}(t)=1\) for a \(t≤K(ξ)T_r(θ)\) exactly if \(\sum_{k=1}^{K(ξ)T_r(θ)}s^{[1]}_i(k)≥q_iT_r(θ)\) which is equivalent to \(\) by~\eqref{eq:3}
